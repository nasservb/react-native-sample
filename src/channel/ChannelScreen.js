...
export default class ChannelScreen extends Component {
  ...
  async loadByUsername(username) {
    try {
      ...
      Alert.alert(
        'خطا',
        'متاسفانه صفجه پیدا نشد',
        [{ text: 'تایید', onPress: () => this.props.navigation.goBack() }],
        { cancelable: false }
      );
    };
  }
  ...
  render() {
    ...
    const header = (
      <View>
        ...
        {!!channel.blocked && <View style={styles.blockView}>
          <Text style={styles.blockText}>شما کاربر @{channel.username} را بلاک کرده اید</Text>

          <Touchable onPress={() => this.props.channel.store(this.guid).toggleBlock()}>
            <Text style={styles.blockTextLink}>خروج از بلاک</Text>
          </Touchable>
        </View>}
      </View>
    );
    ...
    // is 
    // is a blog? use blog card to render
    if(feed.filter == 'services') {
      renderActivity = (row) => {
        return <ServiceCard entity={row.item} navigation={this.props.navigation}/>
      }
    }
    ...
    if (isOwner) {
      emptyMessage = (
        <View style={CommonStyle.centered}>
          ...
          <Button text="پست مطلب جدید" onPress={this.createPost} />
        </View>
      );
    }
