...
export default class ChannelHeader extends Component {
    ...
    if (this.props.me.guid === this.props.channel.channel.guid) {
      return (
        <ButtonCustom
          ...
          accessibilityLabel={this.props.edit ? 'ذخیره تغییرات' : 'ویرایش تنظیمات صفحه'}
          text={this.props.edit ? 'ذخیره' : 'ویرایش'}
      ...
      return (
        <TouchableHighlightCustom
          ...
          accessibilityLabel="ارسال یک پیام برای این صفحه"
        >
          <Text style={{ color: colors.primary }} > ارسال پیام </Text>
        </TouchableHighlightCustom>
      );
  ...
  changeAvatarAction = async () => {
    imagePicker.show('انتخاب تصویر پروفایل', 'photo')
      .then(response => {
        if (response) {
      ...
      <View>
        ...
        <View style={styles.headertextcontainer}>
          <View style={styles.countercontainer}>
            <TouchableHighlightCustom underlayColor="transparent" style={[styles.counter]} onPress={() => { this._navToSubscribers() }}>
              <View style={styles.counter}>
                <Text style={styles.countertitle}>دنبال کنندگان</Text>
                <Text style={styles.countervalue}>{abbrev(channel.subscribers_count, 0)}</Text>
              </View>
            </TouchableHighlightCustom>
            <View style={styles.counter}>
              <Text style={styles.countertitle}>بازدید</Text>
          ...
          {isEditable && <View style={styles.briefdescriptionTextInputView}>
            <TextInput
              placeholder="یک توصیف درباره خودتان بنویسید ..."
              multiline={true}
              underlineColorAndroid='transparent'
