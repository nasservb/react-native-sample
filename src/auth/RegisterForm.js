...
export default class RegisterForm extends Component {
  ...
  validatePassword(value) {
    ...
    if (this.state.password !== value) {
      error.confirmPasswordError = 'رمزها باید یکسان باشند';
    } else {
  ...
  validateTerms(value) {
    ...
    if(!this.state.termsAccepted && this.state.username.length > 4){
      error.termsAcceptedError = 'شما باید قوانین را بپذیرید';
    } else {
    ...
    return (
      <KeyboardAvoidingView behavior='padding'>
        ...
        <CheckBox
          ...
          title={<Text style={ComponentsStyle.terms}> قوانین<Text style={ComponentsStyle.link} onPress={ ()=> Linking.openURL('https://itimcheh.net') }>آی تیمچه </Text> را می پذیرم</Text>}
  ...
  async onPressRegister() {
    ...
    if (this.state.error.confirmPasswordError) {
      return Alert.alert(
        'Oooopps',
        'لطفآ رمز عبورهای یکسان وارد کنید'
      );
    }
